...
UPDATED_SCREEN_FIRMWARE_INSTALLER="${HOME}/display_firmware/screen-firmware.sh"
...
advanced_more() {
    while true; do
        ...
        echo ""
        echo -e "8) Elegoo Image Cleanser Script (NOT for OpenNept4une)"
        echo -e "----------------------------------------------------------${NC}"
        echo ""
...
install_feature() {
    ...
    if [[ $user_input =~ ^[Yy]$ || -z $user_input || $auto_yes = "true" ]]; then
        ...
        if [$feature_name == "Updated Screen Firmware"]; then
...
screen_firmware() {
    install_feature "Updated Screen Firmware" "$UPDATED_SCREEN_FIRMWARE_INSTALLER" "Do you want to install the updated screen firmware? (customizes ui & adds some functionality)"
}
